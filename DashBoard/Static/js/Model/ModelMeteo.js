...
class ModelMeteo {
  ...
  async obtenirMeteoJour() {
    ...
    let dataMétéo = await instanceRecolteMétéo.obtenirDataAPI(
      "https://public.opendatasoft.com/api/explore/v2.1/catalog/datasets/donnees-synop-essentielles-omm/records?select=date%2C%20dd%2Ctc%2Cu%2Ctemps_passe_1%2Ctemps_present%2Ctc%2Cnom_reg&&where=date%20%3D%20date'" +
        this.date +
        "'&limit=100"
    );
    if (dataMétéo["total_count"] === 0) {
      dataMétéo = await instanceRecolteMétéo.obtenirDataAPI(
        "https://public.opendatasoft.com/api/explore/v2.1/catalog/datasets/donnees-synop-essentielles-omm/records?select=date%2Cdd%2Ctc%2Cu%2Ctemps_passe_1%2Ctemps_present%2Cnom_reg%2Cnom_epci&&where=date%20%3D%20date'" +
          this.getJourPrecedent() +
          "'&limit=100"
      );
  ...
  afficherRegion(data) {
    ...
    document.getElementById("temperature").textContent =
      this.formatTemperature(data.tc) + "°C" || "NaN";
    var image_temps = document.getElementById("image_temps");
    const imageSrc =
      (data &&
        data.temps_passe_1 &&
        (data.temps_passe_1.includes("Nuages")
          ? "nuage.png"
          : data.temps_passe_1.includes("Pluie")
          ? "pluie.png"
          : "soleil.png")) ||
      "soleil.png";
  ...
  mettreAJourIFrame(latitude = 48.7784448, longitude = 2.441216) {
    ...
    var nouvelleURL =
      ...
      dateFormatee +
      "/0300Z/wind/surface/level/grid=off/orthographic=" +
      longitude +
      "," +
      latitude +
      ",600";
  ...
  precedentRegion() {
    const index_reg =
      (this.trouverIndexRegion() - 1 + this.data_jour.length) %
      this.data_jour.length;
    this.afficherRegion(this.data_jour[index_reg]);
    this.setRegion(this.data_jour[index_reg]["nom_reg"]);
  }
   ...
   */
  trouverIndexRegion() {
    return this.data_jour.findIndex(
      (element) => element["nom_reg"] === this.region_choisis
    );
  }
